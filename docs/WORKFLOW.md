...
- 当前 MVP 只处理画面字幕，不分析音频。每条画面会先按长边缩放到 960 像素；在本机 1920×1080 合成画面的基准测试中，完整画面单次 OCR 约 0.073 秒，底部 30% 约 0.021 秒，前者约为后者 3.4 倍，实际速度会随 CPU、画面内容和视频解码耗时变化。默认完整画面便于覆盖任意位置的字幕，长视频批处理可改用底部 30% 或自定义范围。
