...
双击字幕文本可以编辑；也可以单选字幕后按 `Enter`——最后点击在字幕列表时直接原地编辑（等同双击该行），最后点击在波形等其它区域时聚焦字幕编辑区。内联编辑时，`Enter` 或 `Ctrl+Enter`（macOS 为 `Cmd+Enter`）按“字幕编辑拆分按键”设置保存或在文字光标处拆分，`Shift+Enter` 插入换行，空格播放或暂停。按 `A`/`W` / `D`/`S` 可跳到上一条 / 下一条字幕的开头并选中它；按 `Shift+A`/`Shift+W` / `Shift+D`/`Shift+S` 则保留当前选择，继续向前 / 后多选一条。播放中跳转会从新位置继续播放，暂停中跳转只移动播放指针。`B` 按指针所在区域拆分字幕：鼠标悬停在已选字幕列表行时按文字位置拆分，位于波形上时按所指的音频位置拆分（与波形右键「按音频位置拆分」一致），其它区域则在红色播放指针处拆分；如果波形入口打开拆分弹窗，关闭主字幕字词时间码或没有可靠字词时间码时会固定使用波形指针的绝对时间，弹窗文字断点只决定左右内容，切点旁会显示时间来源提示。波形工具可按 `V` 切回选择、按 `R` 切换分割。`Alt + 点击` 某条字幕可单独切换禁用状态；禁用项默认不会出现在导出的 SRT 中。

多重字幕开启时，如果当前只选中一条扩展字幕，`B` 会直接打开该扩展字幕的拆分流程；按 `G` 可把当前副字幕绑定到主字幕，按 `Shift+G` 解绑，按 `H` 将已绑定副字幕对齐到主字幕时间轴。`G` 在同时选中一条主字幕时直接绑定，否则会自动拾取唯一的时间重叠主字幕，多个候选或无候选时等待手动选择。副字幕右键菜单中的「在鼠标位置拆分」「绑定到主字幕」「解绑」「对齐主字幕时间范围」分别标注 `B` / `G` / `Shift+G` / `H`。多重字幕工具栏的齿轮中可切换显示方式、主字幕语言类型、副字幕语言类型、跨轨道吸附、同时选中绑定的主副字幕和交换主副字幕；“同时选中主副字幕”默认开启，点击有绑定关系的主字幕或副字幕时会补选绑定的另一条字幕，但字幕编辑区仍以最后点击的字幕为准。单词型适合英语等按空格拆分的语言，字符型适合中文、日文等按字符拆分的语言。列表字数、超长筛选和当前字幕面板会按对应轨道的语言类型计数。跨轨道吸附默认开启，拖动字幕或边界时会把另一条轨道的起止位置加入吸附目标。开启多重字幕时使用齿轮中的「拓展字幕时波形高度」（默认 168px），关闭后恢复「配置」中的波形高度。共享波形双 lane 的标记 `1` / `2` 分别表示主轨 / 扩展轨。
...
当前字幕面板会显示单句时长、字数与阅读速度，便于发现字幕过长或过快。字幕列表可按文本筛选，并可隐藏禁用项。多重字幕关闭时按文本自动判断语言类型；开启时主字幕和副字幕分别使用齿轮中的语言类型：单词型按空白分隔的词计数，字符型按字母/数字字符计数，空白和标点不计入。列表计数、总长度、字/秒和超长过滤使用同一规则。
...
- 多重字幕开启时，主字幕的自动延展和吸收合并会同步处理绑定的副字幕，并保留时间 offset；Alt 临时独立拖动仍是例外。正常调整时以直接操作的轨道为源：主字幕调整不会被副字幕冲突反向缩短，副字幕无法避开冲突时允许重叠并提示；直接拖动副字幕则受主字幕轨道边界限制，主轨没有空间时操作会停止。按 `H` 会把副字幕直接对齐到主字幕完整时间范围，冲突时同样不修改主字幕。
