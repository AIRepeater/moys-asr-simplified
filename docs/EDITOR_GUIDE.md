...
双击字幕文本可以编辑：`Enter` 或 `Ctrl+Enter` 按“字幕编辑拆分按键”设置保存或在文字光标处拆分，`Shift+Enter` 插入换行，空格播放或暂停。按 `A`/`W` / `D`/`S` 可跳到上一条 / 下一条字幕的开头并选中它；按 `Shift+A`/`Shift+W` / `Shift+D`/`Shift+S` 则保留当前选择，继续向前 / 后多选一条。播放中跳转会从新位置继续播放，暂停中跳转只移动播放指针。播放头位于某条字幕内部时，按 `B` 可在波形红色播放指针附近按字/词级时间码拆分该字幕。波形工具可按 `V` 切回选择、按 `R` 切换分割。`Alt + 点击` 某条字幕可单独切换禁用状态；禁用项默认不会出现在导出的 SRT 中。
