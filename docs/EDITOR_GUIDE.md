...
多重字幕开启时，如果当前只选中一条扩展字幕，`B` 会直接打开该扩展字幕的拆分流程。多重字幕工具栏的齿轮中可切换显示方式、扩展字幕拆分类型、跨轨道吸附和交换主副字幕；跨轨道吸附默认开启，拖动字幕或边界时会把另一条轨道的起止位置加入吸附目标。开启多重字幕会临时把波形每行高度调到最大值，关闭后恢复开启前的高度。
