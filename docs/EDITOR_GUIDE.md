...
双击字幕文本可以编辑；也可以先单选字幕，把鼠标留在该字幕行上，再按 `Enter` 原地编辑。内联编辑时，`Enter` 或 `Ctrl+Enter`（macOS 为 `Cmd+Enter`）按“字幕编辑拆分按键”设置保存或在文字光标处拆分，`Shift+Enter` 插入换行，空格播放或暂停。按 `A`/`W` / `D`/`S` 可跳到上一条 / 下一条字幕的开头并选中它；按 `Shift+A`/`Shift+W` / `Shift+D`/`Shift+S` 则保留当前选择，继续向前 / 后多选一条。播放中跳转会从新位置继续播放，暂停中跳转只移动播放指针。单选字幕且鼠标仍位于该列表行时，按 `B` 可在鼠标所指的文字位置拆分；鼠标位于列表外时不会接管 `B` 或 `Enter`。波形工具可按 `V` 切回选择、按 `R` 切换分割。`Alt + 点击` 某条字幕可单独切换禁用状态；禁用项默认不会出现在导出的 SRT 中。
