...
def run_script_match(request: ScriptMatchRequest) -> SubtitleArtifact:
    ...
    if source_project is not None and _has_complete_item_timings(project):
...
def _match_project_with_character_timings(
    ...
    if any(len(symbol) != 1 for symbol in split_punctuation | preserved):
        return _match_project(project, script_text, split_punctuation, preserved)
...
def _match_project(
    ...
    punctuation_segments = _split_script_segments(script_text, split_punctuation, preserve_punctuation)
    alignment_text = "".join(punctuation_segments) if punctuation_segments else script_text
