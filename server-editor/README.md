...
服务器会在本机保存最近 10 个**明确打开过**的 JSON 工程（路径和文件名），工具栏的「最近工程」可直接切换；第一项标为「上次打开」。切换时服务器重新按工程 JSON 的 `media` 字段、或同目录同名前缀的媒体文件规则加载媒体，不会扫描任何目录。带工程路径启动服务器、另存为、从最近列表切换、或在页面中打开工程并被服务器接管（见下）都会更新记录。
...
- 编辑器「导出」设置内的「保存」分组默认开启自动保存，间隔为 30 秒，可调整为 5–3600 秒；只在有可写回工程的服务器版显示。
