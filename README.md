...
- 自带 MAWE 字幕编辑器，支持：
  ...
  - 可显示当前单句的时长、字数和阅读速度，并过滤过长文本；多重字幕可按主/副字幕语言类型分别按词或按字符统计
  - 支持批量替换关键字
  - 视频画面内的字幕预览可直接拖动和缩放
...
- 操作部分
  - WASD 快速跳转前后字幕
  - Enter 聚焦最后点击的主/副字幕编辑区；B 在字幕列表按文字位置、在波形上按音频位置、其它区域按播放指针拆分
  - 多重字幕中，单选副字幕按 G 绑定、Shift+G 解绑、H 对齐到主字幕时间轴、B 打开副字幕拆分；波形双 lane 的 1/2 标记分别表示主轨和扩展轨
