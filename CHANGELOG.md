...
- **纯文本字幕编辑增强** ： 整体编辑不再要求固定字幕行数；新增、删除或移动换行时会按文本内容重新匹配字幕行，在可可靠映射字词时间码时拆分、合并或删除字幕行；没有字词时间码时按原字幕范围和文字长度估算并明确标注。普通清空也会移除字幕。逐条编辑与整体编辑旁同时显示 `⌚️` 有效时间码覆盖率和 `♻️` 原始时间码复用率。
- **整体编辑行宽提示** ： 整体编辑文本框按当前「字数阈值」显示对应的 `em` 竖线；「编辑视图」右侧也提供同一阈值输入，并与字幕列表设置同步。
- **纯文本编辑禁用字幕** ： 默认不显示已禁用字幕，可在编辑窗口开启；隐藏的字幕不会被编辑应用覆盖。
...
- **纯文本编辑性能** ： 输入字幕时延迟重算时间码映射报告，避免每个字符同步触发完整分析；切换视图、筛选和应用时仍会立即刷新。
- **纯文本时间码宽容度** ： 新增标点、符号、emoji 或常见颜文字时保留原有字词时间码，将新增内容并入相邻 item 且不占用独立时长；结构拆分不再因此回退为整句估算。
- **纯文本结构匹配** ： 整体编辑优先使用完整、未修改的字幕行作为匹配锚点，避免局部片段误匹配后把后续未改字幕整体带偏。
