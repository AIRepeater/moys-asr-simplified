...
- 编辑器增加 `Ctrl/Cmd+Shift+A/D` 直接粘合前后字幕、`Esc` 跳过快速上手、波形频谱颜色开关、拖入文件加载进度，以及多重字幕“绑定时自动同步时长”设置（默认开启）。
- 多重字幕预览增加副字幕背景色设置；波形形状来源默认改用媒体旁的最细 `.ReaPeaks` 波形层，没有时回退到自研缓存。

### Changed

- 统一 MAWE Server、便携 HTML 与 Tauri 的前端脚本装配清单，保留现有兼容出口，不改变工程 schema、DOM 合同和编辑器操作。
- HTML 编辑器顶部由生成时间改为显示版本号，并与 Launcher 的版本号保持一致；在线编辑器发布流程会同步使用当前版本。
- mosp/json 拖入已有工程时改为明确选择“打开工程”或“使用工程字幕作为副字幕”；作为副字幕导入时只读取源工程字幕数据，不接管源工程媒体和工作区。
- 多重字幕预览设置面板加宽，副字幕设置与主字幕之间增加分隔线；Ctrl 拖动和 `N` 会按鼠标所在 lane 创建主/副字幕，创建虚影也显示在对应 lane。
- “拼合字幕”工具统一更名为“拼接/合并字幕”，并将拓展方向、参数说明和工具栏提示改为更明确的吸附/合并文案。

### Fixed

- 修复波形取整造成的 1ms 字/词时间码重叠：打开、保存和服务端接管前都会自动修复字/词级异常时间范围，避免正常编辑后的工程保存失败；真正的字幕段重叠仍由严格校验提示。
- 修复 Alt 拖动只能解除联动却不能挤压相邻字幕的问题：Alt 拖动副字幕只挤压副轨，Alt 拖动主字幕会按绑定关系带动副字幕；启用/禁用字幕后立即刷新播放器预览。
- 修复多重字幕指针位于副字幕而没有主字幕时 `B` 仍提示无法拆分的问题；空白波形右键会根据两条轨道当前存在的字幕显示对应的主/副拆分入口。
- 修复 Windows 与 macOS 的 tag 发布工作流并发创建同一个 GitHub Release，导致其中一个平台的资产上传长时间等待的问题。
- 修复多重字幕主轨联动或 `H` 对齐时副轨冲突导致服务端保存失败的问题；冲突时裁剪或删除副字幕并解除绑定，保留较长的剩余区间并保持副轨按时间排序。
- 修复多个主字幕与副字幕重叠时绑定选择不明确的问题：自动绑定时间最早的未绑定主字幕；空白波形菜单按鼠标所在 lane 操作，副轨空白处不再回落拆分主轨。
- 调整多重字幕空白波形右键菜单：按鼠标所在 lane 创建主/副字幕，并根据当前时间点分别显示主字幕和副字幕的音频位置拆分入口；没有对应字幕时入口置灰。
- 修复普通点击副字幕时残留旧主字幕选区的问题；已绑定副字幕只补选其实际绑定的主字幕，显式 Ctrl/Shift 多选仍可用于有意的绑定替换。
- 修复字符型字幕末尾标点被误识别为拆分点，导致拆分弹窗无法提交的问题。
- 修复 LLM 翻译返回单个空字幕或其他不合规 group 时整批失败的问题；现在会跳过坏字幕并提示警告，保留其余合规字幕的输出，全部字幕都坏时不创建空结果，JSON 语法错误则自动重试一次。
- 调整 LLM 后处理批次上限为 80 条字幕或约 4,000 个输入文字字符，并在跳过不合规字幕时报告具体字幕、批次、模型组号、原因和原文摘要。
- 修复旧工程缺少字幕 ID 时浏览器与服务端生成结果不一致，导致媒体接管或保存回归的问题；现在统一使用确定性 ID 生成规则。
- 修复副字幕 `items` 在浏览器归一化、工程保存、服务端校验和主副字幕交换时被清除的问题；`items` 现在保持可选，并支持交换后再次恢复。
- 修复主、副字幕预览刷新节奏和时间区间不一致的问题；补齐副字幕的禁用/隐藏、时间快捷键、跳转播放、选择和波形 A/D 操作，并让边界字幕按 A/D 仍定位到首/末条字幕。
- 修复绑定主字幕的右键菜单缺少“解绑”、联动拆分弹窗不触发字幕忍者反馈，以及 Alt 挤压主字幕后拉回时副字幕范围未恢复的问题。
...
- 多重字幕帮助和文档补充 `G` 绑定、`Shift+G` 解绑、`H` 对齐、`B` 拆分副字幕的说明；副字幕右键菜单显示对应快捷键，波形双 lane 使用 `1` / `2` 标记主轨 / 扩展轨。
- 编辑器增加默认关闭的「多重字幕」双语 MVP：已有主字幕时可拖入 SRT / mosp / JSON 作为扩展轨，按 300ms 容差自动一对一匹配；支持主轨/扩展轨/双列列表、共享波形双 lane、绑定/解绑、联动移动与边界调整、Alt 独立拖动、成对删除、联动合并与近似拆分弹窗，并可独立导出扩展 SRT。
- 多字幕导入弹窗改为先选择行为再确认导入；主字幕替换、多重字幕导入和扩展轨替换均支持撤销。行为按钮默认使用中性样式，选择后才显示选中态。联动拆分弹窗增加字符间 ✂️ 断点、hover 预览与点击锁定，下方预览使用 `/` 分隔并用黄/绿区分两侧；设置项更名为“主字幕自动使用时间码拆分”，关闭后主字幕也使用拆分弹窗并默认定位到时间码位置，开启时联动弹窗以 ⌚️ 标记显示不可交互的主字幕时间码锚点；主字幕缺少可用字词时间码时提示并回退到拆分面板。主轨 C 合并时，未绑定副字幕需与合并范围实际重叠至少 500ms 才会被带入，避免较短的时间码边界误差误合并。
- 调整共享波形中的双字幕 lane 排布：扩展字幕固定在底部，主字幕紧贴在上方，较小行高下也不会发生上下反转或溢出。
- 修复右键菜单「选中后与主字幕绑定」只选中扩展字幕、不执行绑定的问题。
- 扩展字幕右键菜单增加「对齐主字幕时间范围」，支持撤销。
- 扩展字幕波形右键菜单增加「在鼠标位置拆分」：打开独立交互式断点弹窗；绑定段独立拆分时只修改拓展轨并解除原绑定，支持撤销。
- 拆分弹窗的波形入口在关闭主字幕字词时间码或缺少可靠时间码时固定使用鼠标所在的绝对时间；单词型拆分优先在空格或连接两个词的安全标点附近提供断点，并自动清理两侧空格。
- 仅选中连续拓展字幕块时，右键菜单和 `C` 快捷键支持「合并副字幕块」；合并支持撤销，原有绑定会解除并提示重新绑定。
- 视频预览增加主/拓展字幕独立开关、字体、字号和颜色设置；主字幕默认白色，拓展字幕默认黄色且字号小 2px，设置随工程保存。
- 没有扩展轨时多重字幕开关仍可点击，会询问并唤起第二条 SRT 导入；未启用多重字幕时隐藏齿轮设置按钮；齿轮菜单调整下拉框宽度，避免“拓展字幕时波形高度”标签与控件重叠。
- 多重字幕齿轮将“拆分类型”拆为主字幕/副字幕独立的“语言类型”，提供“单词型 / 字符型”提示；字幕列表字数、超长筛选和当前字幕面板会按对应语言类型计数。
- 基础波形开启多重字幕时，主/副两个字幕 lane 使用适度增高的块高，保持高于普通多行模式且低于单主字幕块。
- 选中已绑定的主字幕或副字幕时，共享波形会在两端字幕块右侧显示 🔗 标记。
...
- 提升编辑器成功、警告和无效提示卡片的不透明度，避免提示下方的波形或字幕透出。
- 暂时移除 Windows `MOSE.exe` 与 macOS `MOSE.app` 的 Release 打包，缩小分发包；Launcher 隐藏「在 MOSE 中打开」入口，默认使用 Server 版或 HTML 编辑器。

### Fixed

- 修复「拼合字幕」只调整主字幕时间范围的问题：绑定副字幕现在会随自动起点/终点延展同步调整；主轨 `C` 合并也会优先纳入其绑定副字幕。统一双 lane 标识和菜单文案为「副字幕」。
