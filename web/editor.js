...
function buildStickerOtio() {
  ...
  for (let idx = 0; idx < DATA.segments.length; idx++) {
    ...
    if (!absPath) {
      flashHint('表情包缺少真实磁盘路径；请先设置实际表情包根目录后再导出 OTIO');
      return null;
    }
    const startMs = seg.sticker.start != null ? seg.sticker.start : seg.start;
    const endMs = seg.sticker.end != null ? seg.sticker.end : seg.end;
  ...
  for (const sticker of stickers) {
    ...
    if (startFrame < cursor) {
      flashHint(`表情包时间重叠，无法导出单轨 OTIO：${sticker.name}`);
      return null;
    }
    if (startFrame > cursor) {
      children.push({
