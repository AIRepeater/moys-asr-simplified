(function initMaweI18n(global) {
  ...
  const EN_TEXT = {
    ...
    '无选中时前后跳转 5 秒；有选中时按设置步长移动字幕': 'Seek back/forward 5 sec with no selection; move selected subtitles by the configured step',
    '调整选中字幕左边界（起点）': 'Adjust the selected subtitle left edge (start)',
    '调整选中字幕右边界（终点）': 'Adjust the selected subtitle right edge (end)',
    '按住字幕块或边界时微调；': 'Fine-tune while holding a subtitle block or edge; ',
    '取消黏连联动': 'disable linked-boundary ripple',
    '字幕按键微调步长': 'Subtitle keyboard adjustment step',
    '←/→ 移动字幕；Ctrl+←/→ 调整起点；Ctrl+Shift+←/→ 调整终点；A/D 需先按住字幕块或边界。': '←/→ move subtitles; Ctrl+←/→ adjust the start; Ctrl+Shift+←/→ adjust the end; hold a subtitle block or edge before using A/D.',
