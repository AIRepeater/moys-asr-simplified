(function initMaweI18n(global) {
  ...
  const EN_TEXT = {
    ...
    '多重字幕': 'Multiple subtitles', '主轨': 'Main track', '扩展轨': 'Extension track', '双列': 'Two columns',
    ...
    '主字幕颜色': 'Main subtitle color', '拓展字幕颜色': 'Extension subtitle color',
    ...
    '媒体': 'Media', '预览字幕': 'Subtitle preview', '预览拓展字幕': 'Extension subtitle preview', '预览表情包': 'Sticker preview', '媒体播放控制': 'Media playback controls',
    ...
    '开启时，波形拆分优先使用主字幕的字词时间码；关闭后打开拆分弹窗选择字符间断点。字幕列表拆分不受影响': 'When enabled, waveform splits prefer the main subtitle\'s word timestamps; when disabled, open the split dialog to choose a character boundary. Subtitle-list splits are unchanged',
    ...
    '分割工具（R）：点击字幕块在指针位置安全拆分（默认按字词时间码对齐；关闭设置后打开拆分点弹窗，拒绝 100ms 以内的边缘拆分）；Esc 切回选择': 'Razor tool (R): click a subtitle block to safely split at the pointer (by default aligned to word timestamps; when disabled, open the split-point dialog; reject splits within 100 ms of an edge); Esc returns to Select',
