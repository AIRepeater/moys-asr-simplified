...
(function () {
  ...
  function computeGroupBadges(segments) {
    ...
    const apply = (type, headField, refField) => {
      // 字幕块覆盖层会按可视行重建；先按 head 建索引，避免每个分组
      // 再扫描整份字幕数组。多字幕工程会同时保留主轨和拓展轨，重复扫描
      // 会把一次轻量 UI 更新放大成明显的卡顿。
  ...
  class WaveformEditor {
    ...
    endCueDrag(event) {
      ...
      drag.indices.forEach((idx) => { this.options.getSegments(drag.track)[idx]._dirty = true; });
      this.options.onCommitEdit(drag.indices, drag.kind, drag.track, drag.independent === true);
      this.refreshCueOverlay();
    }
